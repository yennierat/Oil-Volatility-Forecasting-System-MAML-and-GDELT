"""
MAML Dataset Builder v2 — GDELT + Brent Oil Volatility (23-feature dataset with per-region GDELT aggregates)
...
Usage:
    python3 build_maml_dataset_v2.py

Inputs:
    dataset_daily (3).csv
    dataset_intraday (4).csv

Outputs:
    data/dataset_maml_daily_v2.csv
    data/dataset_maml_intraday_v2.csv
    data/maml_task_summary.txt
"""
...
def add_maml_structure(df: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    return df
def add_per_region_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds per-region GDELT aggregates to each row.
    Groups by date + snapshot_hour, computes Middle East,
    oil_producer, oil_consumer sub-aggregates separately.
    """
    print("\n  Adding per-region features...")

    # Assign oil region to each event row
    df["oil_region"] = df["primary_country"].map(assign_oil_region)

    # Group key — date + snapshot_hour if available
    group_cols = ["date"]
    if "snapshot_hour" in df.columns:
        group_cols.append("snapshot_hour")
    ...
    for key, group in df.groupby(group_cols):
        ...
        for region, prefix in [
            ...
            if len(rg) > 0:
                gs = rg["goldstein_scale"].dropna()
                row[f"{prefix}_gs_mean"]      = float(gs.mean()) if len(gs) else 0.0
                row[f"{prefix}_conflict_pct"] = float((gs < 0).mean()) if len(gs) else 0.0
                row[f"{prefix}_n_events"]     = float(len(rg))
                row[f"{prefix}_tone_mean"]    = float(rg["avg_tone"].dropna().mean()) if rg["avg_tone"].notna().any() else -1.0
            else:
                row[f"{prefix}_gs_mean"]      = 0.0
                row[f"{prefix}_conflict_pct"] = 0.0
                row[f"{prefix}_n_events"]     = 0.0
                row[f"{prefix}_tone_mean"]    = -1.0

        region_rows.append(row)

    region_df = pd.DataFrame(region_rows)
    # Safety: if date ended up as tuple from groupby, extract first element
    if len(region_df) > 0 and isinstance(region_df["date"].iloc[0], tuple):
        region_df["date"] = region_df["date"].apply(lambda x: x[0] if isinstance(x, tuple) else x)
    region_df["date"] = pd.to_datetime(region_df["date"])
    df["date"] = pd.to_datetime(df["date"])

    df = df.merge(region_df, on=group_cols, how="left")

    # Fill any nulls with zeros (days with no regional events)
    for prefix in ["me", "oi", "oc"]:
        for suffix in ["gs_mean", "conflict_pct", "n_events", "tone_mean"]:
            col = f"{prefix}_{suffix}"
            df[col] = df[col].fillna(0.0)

    new_cols = [c for c in df.columns if c[:3] in ("me_", "oi_", "oc_")]
    print(f"  Added {len(new_cols)} per-region columns: {new_cols}")
    return df
...
def main():
    ...
    merged_daily    = add_per_region_features(merged_daily)
    ...
    merged_intraday    = add_per_region_features(merged_intraday)
    maml_intraday   = add_maml_structure(merged_intraday, "INTRADAY")

    daily_path    = f"{OUTPUT_DIR}/dataset_maml_daily_v2.csv"
    intraday_path = f"{OUTPUT_DIR}/dataset_maml_intraday_v2.csv"
